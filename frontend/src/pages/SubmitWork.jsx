...
export default function SubmitWork() {
  ...
  // Load real active contracts from backend
  useEffect(() => {
    const token = sessionStorage.getItem("ps_token");
    getContracts({ status: "active" }, token)
      .then(data => setContracts(data.projects ?? data))
      .catch(() => {
        // Fallback demo contracts when backend is offline
        setContracts([
          { id: 1, title: "Logo & Brand Identity",        projectId: 1 },
          { id: 2, title: "Smart Contract Development",   projectId: 2 },
          { id: 4, title: "Mobile App UI Design",         projectId: 4 },
        ]);
      });
  }, []);
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    try {
      // 1. Upload all files to IPFS via Pinata
      const rawFiles = files.map(f => f.file);
      const results  = await uploadFiles(rawFiles, selectedContract);
      const ipfsHash = results[0].ipfsHash;
      ...
      const contract = contracts.find(c => String(c.id ?? c.projectId) === String(selectedContract));
      setUploaded({
        hash: ipfsHash,
      ...
      console.error(err);
      alert(err.message || "Upload failed. Check your Pinata API key and try again.");
    } finally {
      setUploading(false);
    }
  };
